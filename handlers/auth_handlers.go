...
func GenerateSecret(c *gin.Context) {
    ...
    query := 
    `
    MATCH(i:Index {name: "secret_index"})
    SET i.value = i.value + 1
    CREATE(s:Secret {id: i.value, name: $name, secretID: $secretID, secret: $secret }) return s.id as id, s.name as name, s.secretID as secretID, s.secret as secret
    `
    params := map[string]interface{}{
        "name": data.Title,
        "secretID": secretID,
